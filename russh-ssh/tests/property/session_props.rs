...
fn arb_auth_config() -> impl Strategy<Value = AuthConfig> {
    prop_oneof![
        ...
        ("[a-z/_]{5,30}", any::<bool>()).prop_map(|(path, encrypted)| AuthConfig::PublicKey {
...
fn arb_port_forward() -> impl Strategy<Value = PortForward> {
    prop_oneof![
        (1024u16..65535, arb_host(), 1u16..65535).prop_map(
            |(local_port, remote_host, remote_port)| PortForward::Local {
                local_port,
                remote_host,
                remote_port,
            }
        ...
        (
            1u16..65535,
            "localhost|127\\.0\\.0\\.1".prop_map(|s| s.to_string()),
            1024u16..65535
        )
            .prop_map(
                |(remote_port, local_host, local_port)| PortForward::Remote {
                    remote_port,
                    local_host,
                    local_port,
                }
...
fn arb_session_profile() -> impl Strategy<Value = SessionProfile> {
    (
        ...
        .prop_map(
            |(name, host, username, port, auth, timeout_ms, forwards, env, desc, tags)| {
...
mod edge_case_tests {
    ...
    fn empty_profile_name_is_incomplete() {
        let profile =
            SessionProfile::new(String::new(), "host.com".to_string(), "user".to_string());
        assert!(!profile.is_complete());
    }

    #[test]
    fn empty_host_is_incomplete() {
        let profile = SessionProfile::new("Name".to_string(), String::new(), "user".to_string());
        assert!(!profile.is_complete());
    }
    ...
    fn empty_username_is_incomplete() {
        let profile =
            SessionProfile::new("Name".to_string(), "host.com".to_string(), String::new());
        assert!(!profile.is_complete());
    }
